...
class Enemy extends AliveObject {
  ...
  checkZone() {
    if (
      (this.x >= 9 * celPixels &&
        this.y < 6 * celPixels &&
        ((hero.x >= 9 * celPixels && hero.y < 6 * celPixels) ||
          (hero.x >= 15 * celPixels && hero.y < 9 * celPixels))) || //IronHack Room
      (this.x < 9 * celPixels &&
        this.y >= 10 * celPixels &&
        this.y < 15 * celPixels &&
        ((hero.x < 9 * celPixels &&
          hero.y >= 10 * celPixels &&
          hero.y < 15 * celPixels) ||
          (hero.x < 12 * celPixels && hero.y >= 11 * celPixels))) || //First small room
      (this.x < 9 * celPixels &&
        this.y >= 15 * celPixels &&
        this.y < 20 * celPixels &&
        ((hero.x < 9 * celPixels &&
          hero.y >= 15 * celPixels &&
          hero.y < 20 * celPixels) ||
          (hero.x < 12 * celPixels && hero.y >= 16 * celPixels))) || //Second small room
      (this.x < 9 * celPixels &&
        this.y >= 20 * celPixels &&
        this.y < 25 * celPixels &&
        ((hero.x < 9 * celPixels &&
          hero.y >= 20 * celPixels &&
          hero.y < 25 * celPixels) ||
          (hero.x < 12 * celPixels && hero.y >= 21 * celPixels))) || //Third small room
      (this.x >= 12 * celPixels &&
        this.y >= 10 * celPixels &&
        hero.x >= 11 * celPixels &&
        hero.y >= 10 * celPixels) || //Big room
      (this.y >= 5 * celPixels &&
        this.y < 10 * celPixels &&
        hero.y >= 5 * celPixels &&
        hero.y < 10 * celPixels) || // Horizontal corridor
      (this.y >= 9 * celPixels &&
        this.x >= 8 * celPixels &&
        this.x < 12 * celPixels &&
        hero.y >= 6 * celPixels &&
    ...
    if (hero.isAlive()) {
      if (this.isAlive()) {
        ...
        this.path = Pathfinder.findPath(
          nodes,
          nodes[this.row][this.column],
          nodes[hero.row][hero.column]
        );
  ...
  generateItem() {
    ...
    if (random > 0.5) {
      totalItems.push(new GameObject(healthItemImg, this.x, this.y, 13, 13));
    }
  }
}
