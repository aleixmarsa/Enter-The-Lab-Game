function define_pf() {
  var Pathfinder = {
    ...
    util: {
      ...
      lowerFCost: function (node_a, node_b) {
        if (node_a.fCost > node_b.fCost) return -1;
        else if (node_a.fCost < node_b.fCost) return 1;
        return 0;
      },
      //Checks if adjacents nodes (matrix positions) are valid to move into (no 1-wall or 2-obstacle)
      adjacentNodes: function (node, nodes) {
        ...
        if (
          node.x >= 1 &&
          collisionArray[node.x - 1][node.y] !== 1 &&
          collisionArray[node.x - 1][node.y] !== 2
        ) {
        ...
        if (
          node.y > 0 &&
          collisionArray[node.x][node.y - 1] !== 1 &&
          collisionArray[node.x][node.y - 1] !== 2
        ) {
  ...
  Pathfinder.node.prototype.initialize = function (
    parent,
    destination_x,
    destination_y
  ) {
